...
class StarCommunityController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($stars as $k => $v){
            ...
            if($v->id == 1){
                $data[$k]['star_bl'] = bcmul(config('senior_admin.senior_admin_1_reward_bl')*100, 4);
            }else if($v->id == 2){
                $data[$k]['star_bl'] = bcmul(config('senior_admin.senior_admin_2_reward_bl')*100, 4);
            }else if($v->id == 3){
                $data[$k]['star_bl'] = bcmul(config('senior_admin.senior_admin_3_reward_bl')*100, 4);
            }
